...
contract Num_Complex {
    ...
    function fromPolar(SD59x18 r, SD59x18 T) public pure returns (Complex memory a) {
        ...
        if (T.unwrap() > 0) {
            a.re = (r * sd(Trigonometry.cos(uint(T.unwrap()))));
            a.im = (r * sd(Trigonometry.sin(uint(T.unwrap()))));
        } else {
            a.re = (r * sd(Trigonometry.cos(uint(T.unwrap()))));
            a.im = -(r * sd(Trigonometry.sin(uint(-T.unwrap()))));
        }
    ...
    function pow(Complex memory a, SD59x18 n) public pure returns (Complex memory) {
        ...
        a.re = rTOn * sd(Trigonometry.cos(uint(nTheta.unwrap())));
        a.im = rTOn * sd(Trigonometry.sin(uint(nTheta.unwrap())));
